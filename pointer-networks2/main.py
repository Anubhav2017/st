...
flags.DEFINE_boolean('load_from_checkpoint', False, 'Whether to load from checkpoint')
flags.DEFINE_string('checkpoint_dir', 'checkpoints', 'Directory to store checkpoints')
flags.DEFINE_string('log_dir1', 'pointer_logs', 'Directory to put tensorboard log files')
...
class PointerNetwork(object):
    ...
    def step(self):
        ...
        with sess.as_default():
            train_writer = tf.train.SummaryWriter(FLAGS.log_dir1 + "/" + FLAGS.problem_type +"/" + FLAGS.pointer_type+ "/train", sess.graph)
            test_writer = tf.train.SummaryWriter(FLAGS.log_dir1 + "/" + FLAGS.problem_type +"/" + FLAGS.pointer_type + "/test", sess.graph)
