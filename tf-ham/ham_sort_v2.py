...
inputs = np.ndarray((batch_size, n, embed_size)).astype(np.float32)
control = np.ndarray((batch_size, controller_size)).astype(np.float32)
target = np.ndarray((batch_size, n, embed_size)).astype(np.float32)
...
calculate_predicted = tf.concat([tree.get_output(control) for _ in xrange(n)],1)
